...
class Deal(Listing):
    ...
    # ── Live competitive picture (set by core.annotate_live_market from a live
    # /sell/release scrape; present only when LIVE_MARKET is on and the fetch
    # succeeded). `best_alt` (below) doubles as the single best *other* offer.
    is_cheapest: bool = False              # this copy is the cheapest live copy at/above the floor
    market_total: int | None = None        # count of live copies at/above the floor (incl. this one)
    market_low: float | None = None        # cheapest effective landed among those
    market_high: float | None = None       # dearest effective landed among those
    market_copies: list[dict] = field(default_factory=list)  # the rest of the field (ladder rows)
    market_authoritative: bool = False     # live fetch succeeded (claims may be absolute)
    ...
    # Best currently-listed copy of the same release at this grade or better that
    # lands cheaper than this deal (set by core.annotate_best_alt). Dedup-immune:
    # drawn from the current scan, so it surfaces a cheaper copy the re-alert gate
    # hid. Rendered as a distinct second card; None when this deal is already the
    # cheapest at its grade-or-better. The alt itself never carries a best_alt.
    best_alt: "Deal | None" = None
    ...
    def to_pending(self) -> dict:
        ...
        unused once a deal is queued for the digest) — including the nested
        best_alt's, so a queued deal round-trips through JSON."""
        d = dataclasses.asdict(self)
        d.pop("listed_at", None)
        if isinstance(d.get("best_alt"), dict):
            d["best_alt"].pop("listed_at", None)
        return d

    @classmethod
    def from_pending(cls, d: dict) -> "Deal":
        """Rebuild a Deal from a persisted dict, tolerating legacy/renamed keys
        (unknown keys dropped, missing keys defaulted). Rebuilds the nested
        best_alt (persisted as a plain dict) back into a Deal."""
        known = {f.name for f in dataclasses.fields(cls)}
        data = {k: v for k, v in d.items() if k in known}
        if isinstance(data.get("best_alt"), dict):
            data["best_alt"] = cls.from_pending(data["best_alt"])
        return cls(**data)
