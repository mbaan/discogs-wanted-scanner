...
def test_pick_rows_render_discount_badges():
    ...
    html = notifier._combine_html(deal)
    text = "\n".join(notifier._combine_text(deal))
    import re
    ...
    # no-data + legacy rows carry no badge: exactly two signed-% badges render.
    assert len(re.findall(r"[−+]\d+%", html)) == 2
    assert len(re.findall(r"[−+]\d+%", text)) == 2
    assert "#8a5e14" in html                            # discount badge is brass
