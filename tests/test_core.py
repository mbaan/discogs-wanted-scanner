...
def test_best_alt_surfaces_dedup_hidden_cheaper_copy():
    # The cheaper copy (id 1) was alerted before and is gated out of the digest, but
    # it's still listed — so it must resurface as the emitted deal's best_alt. This is
    # the exact "a better copy is still buyable but hidden" case.
    listings = [
        _listing(1, buyer_price=12.0, shipping_buyer_price=1.0),
        _listing(2, buyer_price=16.0, shipping_buyer_price=1.0),
    ]
    sold = {100: _sold_one([10, 20, 30, 40, 50], 30.0)}
    res = core.build_digest(listings, {1: 12.0}, {}, _cfg(sold_price_min_points=5), NOW, sold)
    assert [d.id for d in res.deals] == [2]                 # cheaper copy gated out of digest
    assert res.deals[0].best_alt is not None
    assert res.deals[0].best_alt.id == 1                    # …resurfaced as the best price
    assert res.deals[0].best_alt.sold_median_value == 30.0  # alt carries sold display too


def test_best_alt_none_when_deal_is_already_cheapest():
    listings = [
        _listing(1, buyer_price=12.0, shipping_buyer_price=1.0),
        _listing(2, buyer_price=16.0, shipping_buyer_price=1.0),
    ]
    sold = {100: _sold_one([10, 20, 30, 40, 50], 30.0)}
    res = core.build_digest(listings, {}, {}, _cfg(sold_price_min_points=5), NOW, sold)
    assert res.deals[0].id == 1                             # cheapest is the primary
    assert res.deals[0].best_alt is None


def test_annotate_best_alt_picks_cheapest_same_or_better_grade():
    primary = Deal(id=1, release_id=5, media_condition="Very Good Plus (VG+)", effective_cost=50.0)
    nm_cheaper = Deal(id=2, release_id=5, media_condition="Near Mint (NM or M-)", effective_cost=44.0)
    m_cheapest = Deal(id=5, release_id=5, media_condition="Mint (M)", effective_cost=40.0)
    vg_cheaper = Deal(id=3, release_id=5, media_condition="Very Good (VG)", effective_cost=30.0)  # worse grade
    nm_pricier = Deal(id=4, release_id=5, media_condition="Near Mint (NM or M-)", effective_cost=60.0)
    core.annotate_best_alt([primary], {5: [primary, nm_cheaper, m_cheapest, vg_cheaper, nm_pricier]})
    assert primary.best_alt is not None
    # Cheapest among same-or-better grade (Mint €40); the cheaper-but-worse VG €30 is ignored.
    assert primary.best_alt.id == 5


def test_best_alt_round_trips_through_pending():
    alt = Deal(id=2, release_id=5, media_condition="Near Mint (NM or M-)",
               effective_cost=44.0, buyer_price=44.0)
    primary = Deal(id=1, release_id=5, buyer_price=52.0, best_alt=alt)
    again = Deal.from_pending(primary.to_pending())
    assert isinstance(again.best_alt, Deal)
    assert again.best_alt.id == 2 and again.best_alt.buyer_price == 44.0


NM = "Near Mint (NM or M-)"


def test_gauge_range_trims_outliers():
    """The gauge low/high are display-only and must resist a freak sale stretching
    the bar: with >=5 points they use the 10th–90th percentile, not raw min/max."""
    stats = {7: {"currency": "EUR", "by_condition": {NM: {
        "median": 50.0, "count": 7, "low": 10.0, "high": 200.0,
        "prices": [10, 40, 45, 50, 55, 60, 200],
    }}}}
    deal = Deal(id=1, release_id=7, media_condition=NM)
    core.annotate_sold_price([deal], stats)
    assert deal.sold_median_value == 50.0
    assert deal.sold_low_value == 28.0        # p10, not the €10 outlier
    assert deal.sold_high_value == 116.0      # p90, not the €200 outlier


def test_gauge_range_keeps_raw_when_few_points():
    stats = {7: {"currency": "EUR", "by_condition": {NM: {
        "median": 50.0, "count": 3, "low": 40.0, "high": 60.0, "prices": [40, 50, 60],
    }}}}
    deal = Deal(id=1, release_id=7, media_condition=NM)
    core.annotate_sold_price([deal], stats)
    assert deal.sold_low_value == 40.0        # too few to trim → raw min/max
    assert deal.sold_high_value == 60.0
